...
import { useInfoAlert } from "@/components/InfoAlert/provider";

type Servicio = {
  id: number; planilla: number; sello: number; total: number;
  fondo: string; cliente: string; fecha: string; checkinero: string;
};
...
export default function InfoAlertPrototype() {
  ...
  const filtered = useMemo(() => {
    ...
    let arr = MOCK.filter((r) =>
      !s
        ? true
        : String(r.planilla).includes(s) ||
          String(r.sello).includes(s) ||
          r.fondo.toLowerCase().includes(s) ||
  ...
  const allVisibleChecked = pageData.length > 0 && pageData.every((r) => selected.includes(r.id));

  // Acciones DEMO mostrando InfoAlert
  const abrirLlegadas = () =>
    push({ type: "info", title: "Llegadas", message: "Se abrió el módulo de llegadas (demo)." });

  const abrirProcesoCierre = () =>
    push({ type: "warning", title: "Proceso de cierre", message: "Se inició el proceso de cierre (demo)." });

  const abrirMenuPdfs = () =>
    push({ type: "info", title: "Menú PDFs", message: "Se abrió el menú de PDFs (demo)." });

  const cerrarMenuPdfs = () =>
    push({ type: "error", title: "Menú PDFs", message: "Se cerró el menú de PDFs (demo)." });

  const generarPdf = () => {
    if (!selected.length) {
      push({ type: "warning", title: "Generar PDF", message: "Selecciona al menos un servicio." });
      return;
    }
    push({
      type: "success",
      title: "PDF generado",
      message: `Se generará PDF para ${selected.length} servicio(s) (demo).`,
      duration: 4000,
    });
  };
      ...
      <main className="container mx-auto p-6 space-y-6">
        <section className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="flex gap-4 flex-wrap">
            <button onClick={abrirLlegadas} className="px-5 py-2 rounded-md bg-teal-700 text-white hover:opacity-90">Abrir llegadas</button>
            <button onClick={abrirProcesoCierre} className="px-5 py-2 rounded-md bg-cyan-700 text-white hover:opacity-90">Abrir proceso de Cierre</button>
            <button onClick={abrirMenuPdfs} className="px-5 py-2 rounded-md bg-cyan-700 text-white hover:opacity-90">Abrir Menu Pdfs</button>
            <button onClick={cerrarMenuPdfs} className="px-5 py-2 rounded-md bg-red-800 text-white hover:opacity-90 ml-auto">Cerrar Menu Pdfs</button>
          </div>
        </section>
        ...
        <section className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="flex items-center gap-5">
              <div className="flex items-center gap-3">
                ...
                <label className="inline-flex items-center gap-2">
                  <input type="radio" className="h-4 w-4" checked={groupBy === "fondo"} onChange={() => setGroupBy("fondo")} />
                  <span>Fondo</span>
                </label>
                <label className="inline-flex items-center gap-2">
                  <input type="radio" className="h-4 w-4" checked={groupBy === "cliente"} onChange={() => setGroupBy("cliente")} />
                  <span>Cliente</span>
                </label>
              </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50">
                <tr className="text-left">
                  <th className="p-3 w-10">
                    <input type="checkbox" className="h-4 w-4" checked={allVisibleChecked} onChange={(e) => toggleAllVisible(e.target.checked)} />
                  </th>
              ...
              <tbody>
                {pageData.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.id} className={cx(showDivider && "border-t-4 border-gray-200")}>
                      <td className="p-3">
                        <input type="checkbox" className="h-4 w-4" checked={checked} onChange={(e) => toggleOne(row.id, e.target.checked)} />
                      </td>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button className="px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300 disabled:opacity-50" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Anterior</button>
            <button className="px-4 py-2 rounded-md bg-teal-700 text-white hover:opacity-90 disabled:opacity-50" disabled={page === totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>Siguiente</button>

            <div className="ml-auto w-full md:w-auto">
              <button
                onClick={generarPdf}
                disabled={!selected.length}
                className={cx("w-full md:w-auto px-6 py-3 rounded-md text-white font-medium transition",
